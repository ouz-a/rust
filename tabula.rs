...
impl Error {
    ...
    pub fn io(error: io::Error) -> Self {
        Error { err: Box::new(ErrorImpl { code: ErrorCode::Io(error), line: 0, column: 0 }) }
    }
}
pub fn ret_er(_er: ErrorCode) -> Error {
    let custom_error = OtherError::new(ErrorKind::Other, "oh no!");
    let err_impl = ErrorImpl { code: ErrorCode::Io(custom_error), line: 0, column: 0 };

    Error { err: Box::new(err_impl) }
}
